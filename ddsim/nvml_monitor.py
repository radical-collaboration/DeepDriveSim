...
class NvmlMonitor:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, daemon=True, name="NvmlMonitor"
        )
        self._thread.start()
        print(
            f"[NvmlMonitor] started → {self._output_dir}  "
    ...
    def _init_nvml(self) -> None:
        ...
        try:
            ...
            self._gpu_handles = [
                self._nvml.nvmlDeviceGetHandleByIndex(i) for i in range(n)
            ]
            self._nvml_ok = True
        ...
        if self._nvml_ok:
            for i, handle in enumerate(self._gpu_handles):
                ...
                try:
                    ...
                    metrics[f"{prefix}_memory_percent"] = (
                        round(100 * mem.used / mem.total, 4) if mem.total else 0
                    )
                except Exception:
                    pass
