...
def load_metrics(telemetry_dir):
    ...
    for file in sorted(telemetry_dir.glob("*.json")):
        ...
        for entry in data["metrics"]:
            row = {"timestamp": entry["timestamp"], "hostname": hostname}
...
def main(telemetry_dir: str):
    ...
    all_gpu_cols = [
        c
        for c in df.columns
        if "gpu_" in c and "utilization" in c and "memory" not in c
    ]
    gpu_cols = [c for c in all_gpu_cols]
    ...
    prefix = telemetry_dir.name.split("-")[0]
    output_file = plots_dir / f"{prefix}_utilization.png"
