...
class MiniAppsWorkflowAsyncflow(DDSimManager):
    """
    ...
    def __init__(self, config: dict = None, **kwargs):
        ...
        self.logger.info(f"Task description: {self.task_description}", component=self.name)
    ...
    def register_tasks(self):
        ...
        task_description that sets CUDA_VISIBLE_DEVICES via:
          {"process_template": {"policy": <Policy>, "env": {"CUDA_VISIBLE_DEVICES": "0", ...}}}
    ...
    async def check_train_status(self):
        ...
        filenames = [Path(root_path, f"data_{rank}_{next_iter}.h5") for rank in range(ranks)]

        self.logger.info(
            f"Waiting for {len(filenames)} file to start training... ", component=self.name
        )
        start_trainig = False
        while True:
            if start_trainig:
            ...
            for filename in filenames:
                if not filename.exists():
                    if self.debug:
                        self.logger.info(
                            f"File {filename} not found yet, wait...", component=self.name
                        )
                    start_trainig = False
    ...
    async def train_model(self):
        ...
        if self.debug:
            ...
            self.logger.info(
                f"{len(self.registered_sims)} simulation(s) running....", component=self.name
            )
        await self.training()
        if self.debug:
